 ...
 */
	 ...
	 */
		...
		static int totalDepth(int depthPhase1, int maxDepth) {
			...
			sbyte[] table = CoordCube.Slice_URFtoDLF_Parity_Prun;
			int index = (CoordCube.N_SLICE2 * URFtoDLF[depthPhase1] + FRtoBR[depthPhase1]) * 2 + parity[depthPhase1];
			if ((d1 = (index % 2 == 0) ? (table[index >> 1] & 0x0f) : ((table[index >> 1] & 0xf0) >> 4)) > maxDepthPhase2)
				return -1;
			...
			sbyte[] table1 = CoordCube.Slice_URtoDF_Parity_Prun;
			int index1 = (CoordCube.N_SLICE2 * URtoDF[depthPhase1] + FRtoBR[depthPhase1]) * 2 + parity[depthPhase1];
			if ((d2 = (index1 % 2 == 0) ? (table1[index1 >> 1] & 0x0f) : ((table1[index1 >> 1] & 0xf0) >> 4)) > maxDepthPhase2)
				return -1;
			...
			do {
				...
				sbyte[] table2 = CoordCube.Slice_URtoDF_Parity_Prun;
				int index2 = (CoordCube.N_SLICE2
					* URtoDF[n + 1] + FRtoBR[n + 1])
					* 2 + parity[n + 1];
				sbyte[] table3 = CoordCube.Slice_URFtoDLF_Parity_Prun;
				int index3 = (CoordCube.N_SLICE2
					* URFtoDLF[n + 1] + FRtoBR[n + 1])
					* 2 + parity[n + 1];
				minDistPhase2[n + 1] = Math.Max((index2 % 2 == 0) ? (table2[index2 >> 1] & 0x0f) : ((table2[index2 >> 1] & 0xf0) >> 4), (index3 % 2 == 0) ? (table3[index3 >> 1] & 0x0f) : ((table3[index3 >> 1] & 0xf0) >> 4));
				// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
